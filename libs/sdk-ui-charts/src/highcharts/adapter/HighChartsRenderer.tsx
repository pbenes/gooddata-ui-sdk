...
> {
    ...
    public renderHighcharts(): React.ReactNode {
        ...
        const c1 = this.createChartConfig(this.props.hcOptions, this.state.legendItemsEnabled);
        console.log("zz clone", new Date().getTime() - start);
        start = new Date().getTime();
        const c2 = this.generateConfigPlain(this.props.hcOptions, this.state.legendItemsEnabled);
        console.log("zz plain", new Date().getTime() - start);

        console.log("zz:equal", isEqual(c1, c2), c1, c2);
        const chartProps = {
